...
class AbaPastasE2EFlowTest : BaseTest() {
    ...
    fun testFullFlowAbaPastas(robot: FxRobot) {
        ...
        WaitForAsyncUtils.waitFor(10, TimeUnit.SECONDS) { tbView.items.size == 6 }

        // Ordenar deterministicamente para validação (File.listFiles() não garante ordem)
        val itemsOrdenados = tbView.items.sortedWith(compareBy({ it.volume }, { it.capitulo }, { it.arquivo }))
        ...
        val mockMangaV1 = com.fenix.ordenararquivos.model.entities.Manga().apply {
            nome = "Naruto"
            volume = "1"
            caminhos = mutableListOf(
                com.fenix.ordenararquivos.model.entities.Caminhos("001", "1", "Scan", ""),
                com.fenix.ordenararquivos.model.entities.Caminhos("002", "1", "Scan", "")
            )
        }
        val mockMangaV2 = com.fenix.ordenararquivos.model.entities.Manga().apply {
            nome = "Naruto"
            volume = "2"
            caminhos = mutableListOf(
                com.fenix.ordenararquivos.model.entities.Caminhos("004", "2", "Scan", ""),
                com.fenix.ordenararquivos.model.entities.Caminhos("005", "2", "Scan", "")
            )
        }
        val mockMangaV7 = com.fenix.ordenararquivos.model.entities.Manga().apply {
            nome = "Naruto"
            volume = "7"
            caminhos = mutableListOf(
                com.fenix.ordenararquivos.model.entities.Caminhos("007", "7", "Scan", "")
            )
        }

        whenever(mockMangaService.findAll(eq("Naruto"), any(), any(), any())).thenReturn(listOf(mockMangaV1, mockMangaV2, mockMangaV7))
        ...
        robot.interact { robot.from(tabContent).lookup("#txtPasta").queryAs(JFXTextField::class.java).requestFocus() }
        WaitForAsyncUtils.waitForFxEvents()

        // 4. VALIDAÇÃO DE VOLUMES (SINCRO COM MOCK)
        // Naruto mock: cap 1 -> vol 1, cap 2 -> vol 1, cap 4 -> vol 2, cap 5 -> vol 2, cap 7 -> vol 7.
        // itemsOrdenados:
        ...
        // 5. vol 7 ch 5        -> Cap 5.0  -> Vol 7 (Wait, no, it.volume > 0 initially for this folder)
        ...
        robot.interact {
             robot.from(tabContent).lookup("#tbTabRootPastas").queryAs(com.jfoenix.controls.JFXTabPane::class.java).selectionModel.select(tbTabPastas_ComicInfo)
        }
        WaitForAsyncUtils.waitForFxEvents()

        val mockMal1 = com.fenix.ordenararquivos.model.entities.comicinfo.Mal(1L, "Naruto Original", "Desc 1", null, null, mock())
        val mockMal2 = com.fenix.ordenararquivos.model.entities.comicinfo.Mal(2L, "Naruto Shippuden", "Desc 2", null, null, mock())
        whenever(mockComicInfoService.getMal(anyOrNull(), any())).thenReturn(listOf(mockMal1, mockMal2))
        ...
        robot.clickOn(robot.from(contentComicInfo).lookup("#txtMalNome").query<Node>()).write("Naruto")
        ...
        WaitForAsyncUtils.waitFor(3, TimeUnit.SECONDS) { tbViewMal.items.size == 2 }
        ...
        WaitForAsyncUtils.waitFor(2, TimeUnit.SECONDS) { robot.from(contentComicInfo).lookup("#txtMalId").queryAs(JFXTextField::class.java).text == "1" }
        val txtMalId = robot.from(contentComicInfo).lookup("#txtMalId").queryAs(JFXTextField::class.java)
        assertEquals("1", txtMalId.text)

        // 6. VOLTAR GRID E GERAR CAPAS
        robot.interact {
             robot.from(tabContent).lookup("#tbTabRootPastas").queryAs(com.jfoenix.controls.JFXTabPane::class.java).selectionModel.select(tbTabPastas_Arquivos)
        }
        ...
        WaitForAsyncUtils.waitFor(5, TimeUnit.SECONDS) { tbView.items.any { it.isCapa } }
        val itemsAposCapas = tbView.items.sortedWith(compareBy({ it.volume }, { it.capitulo }, { it.arquivo }))
        
        val capasFisicas = tempDir.toFile().listFiles()?.filter { it.isDirectory && it.name.endsWith("Capa") } ?: emptyList()
        ...
        robot.rightClickOn(robot.from(contentArquivos).lookup(".table-row-cell").atPosition(1).query<Node>())
        robot.clickOn("Aplicar scan nos arquivos proximos") // No controlador está sem acento no 'proximos' desse item
        assertTrue(tbView.items.filter { it.capitulo >= 2.0f }.all { it.scan == "NovaScan" }, "Deveria ter aplicado scan nos proximos")
        ...
        assertTrue(tbView.items.filter { it.capitulo >= 2.0f }.all { it.titulo.isEmpty() }, "Deveria ter apagado titulos")
        ...
        val pastasRenomeadas = tempDir.toFile().listFiles()?.filter { it.isDirectory }?.map { it.name } ?: emptyList()
        assertTrue(pastasRenomeadas.any { it.contains("Capítulo 001") }, "Deveria ter renomeado as pastas")

        // 9. COMPACTAR
        // Recarregar os itens renomeados
        robot.write(tempDir.toAbsolutePath().toString())
        robot.clickOn(robot.from(tabContent).lookup("#btnCarregar").query<Node>())
        WaitForAsyncUtils.waitFor(5, TimeUnit.SECONDS) { tbView.items.isNotEmpty() }
        
        // Mock static Winrar
        val mockWinrar = Mockito.mockStatic(com.fenix.ordenararquivos.process.Winrar::class.java)
        try {
            mockWinrar.`when`<Boolean> { 
                com.fenix.ordenararquivos.process.Winrar.compactar(any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any()) 
            }.thenReturn(true)
            ...
            val btnCompactar = robot.from(tabContent).lookup("#btnCompactar").queryAs(JFXButton::class.java)
            WaitForAsyncUtils.waitFor(10, TimeUnit.SECONDS) { !btnCompactar.isDisabled }

            // Verifica se "tentou" renomear/compactar (os arquivos reais dependeriam do Winrar instalado)
